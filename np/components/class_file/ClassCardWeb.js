...
export default function ClassCard({ classesData }) {
  ...
  const [heartActive, setHeartActive] = useState(
    classesData.reduce((acc, item, index) => {
      acc[index] = false; // 用 index 做為鍵
      return acc;
    }, {})
  );
  ...
  const toggleHeart = (index) => {
    const newHearts = { ...heartActive }; // 複製現有狀態
    newHearts[index] = !newHearts[index]; // 切換指定 index 的狀態
    setHeartActive(newHearts); // 更新狀態
    notify(newHearts[index]); // 顯示通知，傳入當前是加入還是移除
  };
        ...
        return (
          <div className={styles.cardContainer}>
            <div className={styles.imageWrapper}>
              <img
                src={`/images/class-images/${v.image__u_r_l}`}
            ...
            <div className={styles.truffledRiceContainer}>
              <header className={styles.truffledRiceHeader}>
                <p className={styles.truffledRiceTitle}>{v.class_name}</p>
                <svg
                  className={`${styles.heartIcon} ${
                    heartActive[i] ? styles.active : ""
                  }`}
              ...
              <div className={styles.truffledRiceDetails}>
                ...
                <div className={styles.truffledRicePricing}>
                  <p className={styles.truffledRicePrice}>{v.c_price}</p>
                  {/* 目前沒有折價的價錢 */}
              ...
              <div className={styles.truffledRiceMeta}>
                ...
                <div className={styles.truffledRiceActions}>
                  <a
                    ...
                    onMouseDown={() => goClassDetail(v.class__i_d)}
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="18px"
